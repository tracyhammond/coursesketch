function inputListener() {
	...
	**************/
	...
	/**
	 * Initializes the listener with either an element or an elementId
	 */
	this.initialize = function initialize(element) {
		var localElement = false;
		if (typeof element == 'string' || Object.prototype.toString.call(str) == '[object String]') {
			localElement = document.getElementById(element);
		} else if (element instanceof HTMLElement) {
			localElement = element;
		} else {
			throw Error('invalid element exception: given argument is not an element or an element id');
		}
 ...
 * pointerId: what digit this is by number (0th, 1st second) for mice this can be ignored.
 */
function inputEvent(x, y, time, pressure, size, inputType, pointerId) {
	this.x = x;
	this.y = y;
	this.time = time;
