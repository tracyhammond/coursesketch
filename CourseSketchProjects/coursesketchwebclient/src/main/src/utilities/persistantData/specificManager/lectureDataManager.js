function LectureDataManager(parent, advanceDataListener, parentDatabase,
    ...
    function setLectureLocal(lecture, lectureCallback) {
        database.putInLectures(lecture.id, lecture.toBase64(), function(e,
            request) {
    ...
    function setLectureServer(lecture, lectureCallback) {
        sendData.sendDataInsert(CourseSketch.PROTOBUF_UTIL.ItemQuery.LECTURE,
            lecture.toArrayBuffer());
        advanceDataListener.setListener(Request.MessageType.DATA_INSERT,
    ...
    /**
     * Sets a lecture in both server and local databases
     * 
     * @param lecture
     *            lecture item to set
     * @param lectureCallback
     ...
     *            function to be called after server setting is done
     */
    ...
    function getCourseLectures(lectureIds, lectureCallback) {
        if (isUndefined(lectureIds) || lectureIds == null
            || lectureIds.length == 0) {
            lectureCallback(nonExistantValue);
        }
                    ...
                    function(lecture) {
                        ...
                        if (barrier == 0) {
                            if (lectureIdsNotFound.length >= 1) {
                                advanceDataListener
                                    .setListener(
                                        Request.MessageType.DATA_REQUEST,
                                        CourseSketch.PROTOBUF_UTIL.ItemQuery.LECTURE,
                                        function(evt, item) {
                                            var school = CourseSketch.PROTOBUF_UTIL
                                                .getSrlSchoolClass().decode(
                                                    item.data);
                                            ...
                                            for (var i = 0; i < school.lectures.length; i++) {
                                                localScope
                                                    .setLecture(school.lectures[i]);
                                                lecturesFound
                                                    .push(school.lectures[i]);
                                            }
