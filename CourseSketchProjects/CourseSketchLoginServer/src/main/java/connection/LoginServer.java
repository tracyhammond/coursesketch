...
public final class LoginServer extends GeneralConnectionServer {
     ...
     * @param parent {@link connection.LoginServlet}
     */
    ...
    public void onMessage(final Session conn, final Request req) {
        ...
        try {
            ...
            e.printStackTrace();
            send(conn,
                    createLoginResponse(req, null, false, INCORRECT_LOGIN_MESSAGE,
                            false, null));
        }
    }

    /**
     * Registers the user than attempts to log in the user using the newly created information.
     * @param conn
     *            the session object that created the message
     * @param req
     *            the message itself
     * @param login
     *            the information about the user attempting to log in and how they are attempting to log in.
     */
    private void registerUserMessage(final Session conn, final Request req, final LoginInformation login) {
        try {
            // registers user
            DatabaseClient.createUser(login
                    .getUsername(), login.getPassword(), login.getEmail(), login
                    .getIsInstructor());
            ...
            e.printStackTrace();
            send(conn,
                    createLoginResponse(req, login, false,
    ...
    private void loginUser(final Session conn, final Request req, final LoginInformation login) {
        ...
        try {
            final String userLoggedIn = DatabaseClient.mongoIdentify(login.getUsername(),
                    login.getPassword(), loginAsDefault, login.getIsInstructor());
     ...
     * @param login the information of what the user is attempting to do.
    ...
    private static Request createLoginResponse(final Request req,
            final LoginInformation login, final boolean success,
