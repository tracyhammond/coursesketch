...
import integration.connections.Client1;
...
public final class ProxyServerWebSocketHandler extends ServerWebSocketHandler {
    ...
    public ProxyServerWebSocketHandler(final ServerWebSocketInitializer parent) {
        ...
        final ActionListener listener = new ActionListener() {
            ...
            public void actionPerformed(final ActionEvent event) {
                try {
                    getConnectionManager().send(TimeManager.serverSendTimeToClient(), null, Client1.class);
    ...
    public void onMessage(final SocketSession conn, final Request req) {
        final FakeLoginConnectionState state = (FakeLoginConnectionState) getConnectionToId().get(conn);
        ...
        if (!state.isLoggedIn() && req.getRequestType() != MessageType.TIME) {
            ...
            try {
                this.getConnectionManager().send(req, sessionID, Client1.class);
            } catch (ConnectionException e) {
                send(conn, createBadConnectionResponse(req, Client1.class));
            }
        } else {
            if (state.getTries() > MAX_LOGIN_TRIES) {
                conn.close(STATE_INVALID_LOGIN, INVALID_LOGIN_MESSAGE);
    ...
    public MultiConnectionState getUniqueState() {
        return new ProxyConnectionStateFake(Encoder.nextID().toString());
    }
    ...
    public void initializeListeners() {
        ...
        this.getConnectionManager().setFailedSocketListener(listen, Client1.class);
    }
}
