...
public class Checksum {
    public static final long MAX_TIME_SIZE = 2L << 63L - 1L; // 2 ^ 63 - 1 (or
                                                             // Long.maxValue)
    public static final long LONG_EXP = 64;
    public static final int MAX_LIST_SIZE_BITS = 16; // max size a list can be
                                                     // in a checksum this is 2
                                                     // ^ 24 (which is the
                                                     // midpoint between 16 and
                                                     // 32
    public static final int MAX_LIST_SIZE = 2 << MAX_LIST_SIZE_BITS; // max size
                                                                     // a list
                                                                     // can be
                                                                     // in a
                                                                     // checksum
                                                                     ...
                                                                     // 16 and
                                                                     // 32
    public static final int MAX_COMMAND_SIZE = 2 << 16; // max size that the
                                                        // command bytes can
                                                        // take up
    public static final long MAX_UPDATE_DATA_SIZE = 2L << 32L; // max size that
                                                               // the command
                                                               // bytes can take
                                                               // up
     ...
     *
     * For any two list that are not the same. They should not appear the same
     * (unless every item happened at the same instance and have the exact same
     * id.
     *
     * The checksum is composed of the following in two steps The first step:
     * <ul>
    ...
    public static void main(final String args[]) {
        ...
        final long totalTime = (currentTime * 2) % MAX_TIME_SIZE;
        System.out.println("totalTime\t" + totalTime);
        final long temp1 = ((long) size) << (64 - log2(size));
        ...
        final long result = ((long) size) << (64 - log2(size)) | totalTime;
        System.out.println("the total \t" + Long.toBinaryString(result));
        System.out.println("the total \t" + Long.toBinaryString(-1));

        final SrlChecksum.Builder build1 = SrlChecksum.newBuilder();
        final SrlChecksum.Builder build2 = SrlChecksum.newBuilder();
