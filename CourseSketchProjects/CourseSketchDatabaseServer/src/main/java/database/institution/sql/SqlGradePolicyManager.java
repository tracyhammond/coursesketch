        ...
        try (PreparedStatement stmt = conn.prepareStatement(query, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
            ...
            for (int i = 0; i < categories.size(); i++) {
                ...
                try (final ResultSet rst = stmt.executeQuery()) {
                    if (rst.next()) {
                        rst.updateString(DatabaseStringConstants.GRADE_CATEGORY_NAME, currentCategory.getName());
                        rst.updateFloat(DatabaseStringConstants.GRADE_CATEGORY_WEIGHT, currentCategory.getWeight());
